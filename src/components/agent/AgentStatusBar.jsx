...
export default function AgentStatusBar({ config, running, onRun, onTogglePause }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200/80 shadow-sm px-4 md:px-5 py-4 flex flex-wrap items-center gap-3">
      ...
      <div className="w-full sm:w-auto sm:ml-auto flex items-center gap-2">
        <Button variant="outline" size="sm" onClick={onTogglePause} className="flex-1 sm:flex-none rounded-full">
          {config.paused ? <Play className="w-3.5 h-3.5 mr-1.5" /> : <Pause className="w-3.5 h-3.5 mr-1.5" />}
          {config.paused ? 'Resume' : 'Pause'}
        </Button>
        <Button size="sm" onClick={onRun} disabled={running || config.paused} className="flex-1 sm:flex-none rounded-full bg-[#101418] hover:bg-stone-700">
